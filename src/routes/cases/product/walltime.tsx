...
export const routeData = {
  ...
  localization: T({
    ru: {
      ...
      textDescriptionProduct:'Мобильное приложение, которое превращает обои на экране устройства в живое визуальное повествование. Здесь не просто меняются картинки — смена фонов становится частью настроения, времени суток и эстетики пользователя. walltime предлагает тщательно подобранные коллекции, а также даёт свободу создавать и делиться своими собственными. Приложение было выпущено только под платформу Android, а версия визуала которая используется в кейсе, создавалась под iOS и является лишь выдумкой автора.',

      headingOnboarding:'ОНБОРДИНГ',
      textDescriptionOnbording:'При первом запуске пользователь видит серию слайдов с описанием ключевых возможностей walltime — смена обоев по времени, загрузка собственных наборов, публикация и обмен. Это быстро вводит пользователя в суть.',

      headingHome:'ИСТОРИИ',
      textDescriptionHome:'Главный экран построен по принципу визуального фида. Вверху — «сторис» с новостями, ниже — карточки коллекций, которые можно листать свайпом вверх-вниз. Переключение между глобальными и личными наборами — одним нажатием.',
      ...
      textDescriptionSeeCollection:'Страница, где каждая коллекция раскрывается полностью — с изображениями, автором, категорией и атмосферой. Отсюда легко установить её на экран или мгновенно поделиться с другими.',

      headingMainPublicCollection:'ПУБЛИКАЦИЯ КОЛЛЕКЦИИ',
      textDescriptionPublicCollection:'Перед публикацией достаточно выбрать обложку, указать категорию, добавить описание и задать формат доступа — бесплатный или платный. После оформления и подтверждения ваша коллекция станет доступна другим пользователям в магазине приложения.',

      headingMainSharewall:'SHAREWALL',
      textDescriptionSharewall:'Один пользователь создаёт QR-код для своей коллекции, а другому достаточно просто навести камеру — и подборка мгновенно окажется у него в приложении. Всё происходит без лишних действий, будто по волшебству. При желании доступ можно отключить в любой момент.',
      
      headingMainFeed:'ПОЛЬЗОВАТЕЛЬСКАЯ ЛЕНТА КОЛЛЕКЦИЙ',
      textDescriptionFeed:'Живая галерея, где пользователи делятся своими визуальными мирами. Каждый набор — это чей-то взгляд на стиль, настроение или момент, которым можно вдохновиться и сразу установить себе.',

      headingMainStore:'МАГАЗИН',
      textDescriptionStore:'Пространство с подборками от авторов со всего мира и самой команды приложения, доступными бесплатно или за небольшую плату. Здесь легко найти уникальные визуальные коллекции и установить их в один клик.',

      headingMainProfile:'ПРОФИЛЬ',
      textDescriptionProfile:'Пространство, где собраны твои коллекции, лайки и подписчики — всё, что говорит о тебе через визуальный стиль. Профиль становится отражением вкуса и местом, откуда легко делиться вдохновением с другими.',

      headingMainSettings:'НАСТРОЙКИ',
      textDescriptionSettings:'Раздел настроек позволяет управлять памятью устройства, языком интерфейса, дополнительными функциями, оплатами и поддержкой. Здесь собраны все необходимые инструменты для комфортного использования приложения.',
      
      headingMainPopup:'ДИАЛОГОВЫЕ ОКНА',
      textDescriptionPopup:'Каждое важное действие сопровождается понятным и лаконичным диалогом — будь то удаление коллекции, оценка приложения или выход. Интерфейс построен так, чтобы пользователь всегда чувствовал контроль и понимание происходящего.',

      headingMainTypo:'ТИПОГРАФИКА И СТИЛИ',
      textDescriptionTypo:'В основе интерфейса лежит шрифт SF Pro — современный, технологичный и легко читаемый. Цветовая палитра выстроена на контрасте света и тени, где акцентный градиент оттеняет важные детали, а базовые тона создают глубокую и выразительную визуальную среду.',
    ...
    en: {
      ...
      textDescriptionProduct: 'A mobile application that turns the wallpaper on the device screen into a live visual narrative. The images dont just change here — changing backgrounds becomes part of the users mood, time of day, and aesthetics. Walltime offers carefully selected collections, as well as the freedom to create and share your own. The application was released only for the Android platform, and the visual version used in the case was created for iOS and is only an invention of the author.',

      headingOnboarding:'ONBOARDING',
      textDescriptionOnbording:'At the first launch, the user sees a series of slides describing the key features of walltime — changing wallpapers in time, uploading their own sets, publishing and sharing. This quickly introduces the user to the point.',

      headingHome:'STORIES',
      textDescriptionHome:'The main screen is based on the principle of a visual feed. At the top is a "story" with news, below are collection cards that can be scrolled up and down. Switch between global and personal sets with a single tap.',
      ...
      textDescriptionSeeCollection:'A page where each collection is fully revealed — with images, author, category, and atmosphere. From here, its easy to install it on the screen or instantly share it with others.',

      headingMainPublicCollection:'PUBLISHING A COLLECTION',
      textDescriptionPublicCollection:'Before publishing, it is enough to select a cover, specify a category, add a description and set the access format — free or paid. After registration and confirmation, your collection will be available to other users in the app store.',

      headingMainSharewall:'SHAREWALL',
      textDescriptionSharewall:'One user creates a QR code for his collection, and another just needs to point the camera and the collection will instantly appear in his application. Everything happens without unnecessary actions, as if by magic. If desired, access can be disabled at any time.',
      
      headingMainFeed:'CUSTOM COLLECTIONS FEED',
      textDescriptionFeed:'A live gallery where users share their visual worlds. Each set is someones take on a style, mood, or moment that you can be inspired by and immediately set for yourself.',

      headingMainStore:'STORE',
      textDescriptionStore:'A space with collections from authors from all over the world and the app team itself, available for free or for a small fee. Its easy to find unique visual collections here and install them in one click.',
      
      headingMainProfile:'PROFILE',
      textDescriptionProfile:'The space where your collections, likes, and followers are collected is everything that speaks about you through a visual style. The profile becomes a reflection of taste and a place from where it is easy to share inspiration with others.',

      headingMainSettings:'SETTINGS',
      textDescriptionSettings:'The settings section allows you to manage the devices memory, interface language, additional functions, payments, and support. Here you will find all the necessary tools for comfortable use of the application.',
      
      headingMainPopup:'POPUP',
      textDescriptionPopup:'Every important action is accompanied by a clear and concise dialogue, whether its deleting a collection, evaluating an app, or exiting. The interface is designed so that the user always feels in control and understands what is happening.',

      headingMainTypo:'TYPOGRAPHY & STYLES',
      textDescriptionTypo:'The interface is based on the SF Pro font, which is modern, technological and easy to read. The color palette is based on the contrast of light and shadow, where the accent gradient highlights important details, and the base tones create a deep and expressive visual environment.',
